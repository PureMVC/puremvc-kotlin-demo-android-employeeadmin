...
class StartupCommand: SimpleCommand() {
    ...
    override fun execute(notification: INotification) {
        ...
        val connection = object: SQLiteOpenHelper(application.applicationContext, "employeeadmin.db", null, 1) {
            ...
            override fun onCreate(db: SQLiteDatabase?) {
                ...
                db?.execSQL("CREATE TABLE employee(id INTEGER PRIMARY KEY, username TEXT NOT NULL UNIQUE, first TEXT NOT NULL, last TEXT NOT NULL, email TEXT NOT NULL, password TEXT NOT NULL, departmentId INTEGER NOT NULL, FOREIGN KEY(departmentId) REFERENCES department(id) ON DELETE CASCADE ON UPDATE NO ACTION)")
                db?.execSQL("INSERT INTO employee(id, username, first, last, email, password, departmentId) VALUES(1, 'lstooge', 'Larry', 'Stooge', 'larry@stooges.com', 'ijk456', 1), (2, 'cstooge', 'Curly', 'Stooge', 'curly@stooges.com', 'xyz987', 2), (3, 'mstooge', 'Moe', 'Stooge', 'moe@stooges.com', 'abc123', 3)")

                db?.execSQL("CREATE TABLE employee_role(employee_id INTEGER NOT NULL, role_id INTEGER NOT NULL, PRIMARY KEY(employee_id, role_id), FOREIGN KEY(employee_id) REFERENCES employee(id) ON DELETE CASCADE ON UPDATE NO ACTION, FOREIGN KEY(role_id) REFERENCES role(id) ON DELETE CASCADE ON UPDATE NO ACTION)")
                db?.execSQL("INSERT INTO employee_role(employee_id, role_id) VALUES(1, 4), (2, 3), (2, 5), (3, 8), (3, 10), (3, 13)")
            }
