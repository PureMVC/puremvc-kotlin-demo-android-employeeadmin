...
import org.puremvc.kotlin.demos.android.employeeadmin.business.AppDatabase
import org.puremvc.kotlin.demos.android.employeeadmin.domain.data.AppDatabase
...
import org.puremvc.kotlin.demos.android.employeeadmin.domain.entity.UserRoleJoin

class RoleProxyTest: TestCase() {

    private lateinit var database: AppDatabase

    private lateinit var roleProxy: RoleRepository

    @Before
    override fun setUp() {
        database = Room.inMemoryDatabaseBuilder(ApplicationProvider.getApplicationContext(), AppDatabase::class.java).build()
        roleProxy = RoleRepository(database.roleDAO(), handler)
        ...
        database.roleDAO().insertAll(listOf(
